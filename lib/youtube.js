...
async function handleOAuthCallback({ code, arreUserId }) {
  ...
  // channel name is cosmetic — don't let this kill the OAuth flow
  ...
  try {
    ...
    if (channel) {
      ...
      // customUrl is the @handle (e.g. "@arrevoiceentertainment")
      channelHandle = channel.snippet?.customUrl || null;
    }
  } catch (e) {
    console.warn('Could not fetch YouTube channel info (non-fatal):', e.message);
  }
...
async function getAuthedClient(arreUserId) {
  ...
  // Persist any refreshed token back to Supabase
  ...
  // Proactively refresh if token is expired or expires within next 5 minutes
  const expiresAt  = tokenRow.expires_at ? new Date(tokenRow.expires_at).getTime() : 0;
  const fiveMinMs  = 5 * 60 * 1000;
  const isExpired  = expiresAt < Date.now() + fiveMinMs;

  if (isExpired && tokenRow.refresh_token) {
    try {
      const { credentials } = await client.refreshAccessToken();
      client.setCredentials(credentials);
      // listener above will persist the new token
    } catch (e) {
      console.warn('YouTube proactive token refresh failed:', e.message);
      // proceed anyway — googleapis will attempt refresh during the API call
    }
  }
