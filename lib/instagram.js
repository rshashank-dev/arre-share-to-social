...
function getAuthUrl() {
  ...
  return `${FB_OAUTH}/authorize?${params.toString()}`;
}
...
async function handleOAuthCallback({ code, arreUserId }) {
  ...
  if (shortData.error) throw new Error(`Instagram token exchange failed: ${shortData.error.message}`);
...
const MEDIA_TYPE_MAP = { reel: 'REELS', post: 'VIDEO', story: 'STORIES' };

/**
 * Publishes a video to Instagram (Reel, Feed Post, or Story).
 * @param {object} opts
 * @param {string} opts.arreUserId
 * @param {string} opts.videoUrl - public CDN URL of the audiogram MP4
 * @param {string} opts.caption
 * @param {'reel'|'post'|'story'} opts.format
